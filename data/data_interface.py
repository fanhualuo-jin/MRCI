        ...
        if p < thresh_hold or not self.train:
            for i, sample in enumerate(batch):
                ...
                if '[HistoryHere]' in input_text:
                    insert_prompt = ", ".join(
                        [f"{seq_title} [HistoryEmb] [ImageHere]" for seq_title in sample['seq_name']])
                    input_text = input_text.replace('[HistoryHere]', insert_prompt)
                if '[CansHere]' in input_text:
                    insert_prompt = ", ".join(
                        [f"{can_title} [CansEmb] [CansImage]" for can_title in sample['cans_name']])
...
class DInterface(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        return DataLoader(self.trainset,
                          ...
                          shuffle=True,
                          drop_last=True,
                          collate_fn=TrainCollater(prompt_list=self.prompt_list, llm_tokenizer=self.llm_tokenizer,
                                                   train=True, max_step=self.max_steps))
